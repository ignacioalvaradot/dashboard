  ...
  return (
    ...
    <Grid container justifyContent="center" m={1} >
     <Grid container justifyContent="center" m={1} mb ={1}>
        <Box  width={700} >
      <Slider aria-label="Volume" value={value} onChange={slideChange} max={(metricasHabla.length)} /> 
...
<IconButton color="primary" onClick={() => (setEstado(true), setSlidedata(datas[value]))}>
    ...
    <Box sx={style3} mr={2}> Gráficos de actividad vocal</Box> 
    <HtmlTooltip
        title={
          <React.Fragment>
            <Typography color="inherit">Este es un gráfico de red que busca la relacion entre los miembros del grupo</Typography>
          </React.Fragment>
        }
      >
    <IconButton color="primary">
      ...
      <div key={i}>
        ...
        <Grid item xs={2.3} mr={6} key={i}>
          ...
          <div style={{ textAlign: "center" }}>
            <Button
              sx={{ width: "270px", height: "270px",textTransform: "none" }}
              onClick={() => {
                setSelectedItem(i);
              }}
            >
    ...
    Grupo {i + 1}
              ...
              <NetworkSpeechGraph data={canales} grupos = {i}> </NetworkSpeechGraph>
              </Paper>
            </Button>
          </div>
          <Modal
          ...
          >
            <Box sx={style}>
              ...
              <div style={{ textAlign: "center" }}><ModalLegend data={i}></ModalLegend></div>
              <Grid container spacing={7}>
              <Grid item xs={5}  px={5} py={2}>
              <DoublePieChart data={canales}
                width={270}
                height={270}
                innerRadius={55}
                outerRadius={100}
                grupos = {i}></DoublePieChart></Grid>

                  {/* <Grid item xs={4}  px={6} py={2}>
              <MultilineGraph data={canales}></MultilineGraph></Grid>  */}  
                   <Grid item xs={5}  px={5} py={2}>
              <StackedBarChart data={canales} grupos = {i}></StackedBarChart></Grid>  
              </Grid>
            </Box>
          </Modal>
        </Grid>
