...
var strokeWidth = [];
    data.channel.map((canales, i) => (
      ...
      data.trace_delta.map((canales, i) => (
  ...
  var maxWidth= d3.max(strokeWidth, function(d) { return d;}); 
    ...
    const lineaTransparency = d3.scaleLinear()
    .domain([0 ,maxWidth])
    .range([0,1]); 
    ...
    const nodo = svg
      ...
      .attr("cx", function (d, i) {
       /*  return (
        ...
        return (
          (dimensions.width / 4) *
          Math.cos(2 * Math.PI * ((d.channelId - 1) / dataExp.fase[dataExp.experimento.faseActiva].idGrupos[grupos].participantes.length + 0.75))
        );
      })
 ...
 */
        return (
          (dimensions.height / 4) *
          Math.sin(2 * Math.PI * ((d.channelId - 1) / dataExp.fase[dataExp.experimento.faseActiva].idGrupos[grupos].participantes.length + 0.75))
        );
      })
    ...
    const linea = svg
      ...
      .data(dataExp.fase[dataExp.experimento.faseActiva].idGrupos[grupos].participantes)
      .join("text")
